  ...
  observeEvent(input$predict_species_button, {
    ...
    if (!is.null(response) && httr::status_code(response) == 200) {
      ...
      if (!is.null(result$top_3_predictions)) {
        ...
        valid_predictions <- Filter(function(pred) pred$probability > 0, result$top_3_predictions)
        
        output$species_prediction_output <- renderUI({
          tagList(
            lapply(valid_predictions, function(pred) {
